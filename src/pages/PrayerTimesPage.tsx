    ...
    <div className="min-h-screen">
      <div className="gradient-emerald px-4 pb-8 pt-12 islamic-pattern">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setShowSettings(!showSettings)} className="flex items-center gap-2 rounded-full bg-primary-foreground/20 px-3 py-1.5 text-primary-foreground">
            <Settings size={16} /><span className="text-xs font-medium">{t("common.settings")}</span>
          </button>
        </div>
        <h1 className="text-2xl font-bold text-primary-foreground">{t("prayer.title")}</h1>
        <p className="mt-1 text-sm text-primary-foreground/70">{new Date().toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</p>
      </div>

      {showSettings && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mx-4 -mt-4 mb-3 rounded-xl bg-card p-4 shadow-lg border border-border z-10 relative">
          <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2"><Bell size={16} className="text-primary" />{t("prayer.adhanSettings")}</h3>
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-sm font-medium text-foreground">{t("prayer.enableAdhan")}</p>
              <p className="text-xs text-muted-foreground">{settings.enabled ? (notificationGranted ? `✅ ${t("prayer.activeWithNotifications")}` : `🔊 ${t("prayer.audioOnly")}`) : t("prayer.tapToEnable")}</p>
            </div>
            ...
            <>
              <div className="mb-4">
                ...
                <Select value={settings.selectedAdhan} onValueChange={(v) => updateSettings({ selectedAdhan: v })}>
                  ...
                  <SelectContent>{ADHAN_OPTIONS.map((opt) => (<SelectItem key={opt.id} value={opt.id}>{opt.label}</SelectItem>))}</SelectContent>
                </Select>
              </div>
              <div className="mb-4">
              ...
              <div className="flex gap-2">
                <button onClick={isPlaying ? stopPlayback : testAdhan} className="flex-1 flex items-center justify-center gap-2 rounded-lg gradient-emerald px-4 py-2.5 text-sm font-medium text-primary-foreground">
                  {isPlaying ? (<><Square size={14} /> {t("common.stop")}</>) : (<><Play size={14} /> {t("common.testAdhan")}</>) }
                </button>
              </div>
      ...
      <div className={`px-4 ${showSettings ? "mt-3" : "-mt-4"} space-y-3 pb-6`}>
        {PRAYER_TIMES.map((prayer, i) => {
          ...
          return (
            <motion.div key={prayer.nameKey} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }} className="flex items-center justify-between rounded-xl bg-card p-4 shadow-sm">
              ...
              <div className="flex items-center gap-3">
                {key && settings.enabled && settings.prayers[key] ? <Bell size={14} className="text-primary" /> : <BellOff size={14} className="text-muted-foreground/40" />}
