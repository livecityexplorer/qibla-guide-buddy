...
import { ArrowLeft, ShieldAlert, ShieldCheck, Search, Filter, ChevronDown } from "lucide-react";
...
const BoycottDirectoryPage = () => {
  ...
  const browsableList = useMemo(() => {
    ...
    if (selectedLevel !== "all") list = list.filter(e => e.level === selectedLevel);
    return list;
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="bg-gradient-to-br from-orange-800 via-red-800 to-red-900 px-4 pb-8 pt-12">
        ...
        <p className="mt-1 text-sm text-white/70">{boycottDirectory.length} brands in the boycott list</p>
      </div>
      ...
      <div className="px-4 -mt-4 pb-24 space-y-4">
        ...
        <div className="rounded-2xl bg-card p-4 shadow-sm border border-border">
          ...
          >
            {scanning ? "Searching..." : "Search Directory"}
          </button>
        </div>
          ...
          <div className="text-center">
            {results.length === 0 ? (
              <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">"{query}" was not found.</p>
              </div>
            ) : (
        ...
        {results === null && (
          <div>
            <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {categories.map(c => (
                          <button key={c} onClick={() => setSelectedCategory(c)}
                            className={`rounded-full px-3 py-1 text-xs font-medium transition-all ${selectedCategory === c ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>
                            {c}
                          </button>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {levels.map(l => (
                          <button key={l} onClick={() => setSelectedLevel(l)}
                            className={`rounded-full px-3 py-1 text-xs font-medium capitalize transition-all ${selectedLevel === l ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>
                            {l === "all" ? "All Levels" : l}
                          </button>
                        ))}
                      </div>
        ...
        <div className="space-y-3">
          {displayList.map((entry) => {
            ...
            return (
              <motion.div key={entry.id} layout className={`rounded-2xl border ${config.border} ${config.bg} overflow-hidden`}>
                <button onClick={() => setExpandedId(isExpanded ? null : entry.id)} className="w-full p-4 text-left">
                  <div className="flex items-center gap-3">
                    {entry.logo ? (
                      <img src={entry.logo} alt={entry.name} className="h-14 w-14 rounded-xl object-contain bg-white p-1.5 shadow-sm"
                  ...
                  {isExpanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <div className="px-4 pb-4 space-y-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          <span className="rounded-full bg-secondary px-2.5 py-0.5 text-[10px] font-medium text-secondary-foreground">{entry.category}</span>
                        </div>
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {subBrandNames.map(name => (
                                <span key={name} className="rounded-full border border-destructive/20 bg-destructive/5 px-2.5 py-0.5 text-[10px] font-medium text-destructive">{name}</span>
                              ))}
                            </div>
                          </div>
                        )}
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {relatedNames.map(name => (
                                <span key={name} className="rounded-full border border-muted-foreground/20 bg-muted px-2.5 py-0.5 text-[10px] font-medium text-muted-foreground">{name}</span>
                              ))}
                            </div>
                          </div>
                        )}
...
export default BoycottDirectoryPage;
